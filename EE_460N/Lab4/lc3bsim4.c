...
#define VECTOR_EXC_PROTECTION 0x02
#define VECTOR_EXC_UNALIGNED_ACCESS 0x03
#define VECTOR_EXC_INVALID_OPCODE 0x04
...
void initialize(char *ucode_filename, char *program_filename, int num_prog_files) { 
    ...
    /* CURRENT_LATCHES.REGS[6] = ADDR_USP; /* Initial value of R6 = USP */
...
void eval_micro_sequencer() {
	...
	if (ird == 0) {	/* next state is from J bits */
		...
		else if (cond == 4) {	/* interrupt/exception */
			if (CURRENT_LATCHES.INT == 1 || (CURRENT_LATCHES.EXC & 0x1) == 1) {
				j |= 0x8;	/* set bit 4 */
			}
			if (((CURRENT_LATCHES.EXC >> 1) & 0x1) == 1) {
				j |= 0x10;	/* set bit 5 */
			}
		}
		nextState = j;
...
*/
void latch_datapath_values() {
	if (CYCLE_COUNT == 300) { NEXT_LATCHES.INT = 1; }	/* trigger the interrupt */
...
void computeVectorMUX(void) {
    ...
    else if (sel == 1) {    /* x02 = Protection Exception Vector */
    ...
    else if (sel == 3) {    /* x04 = Invalid Opcode Vector */
...
void computeEXCMUX(void) {
    ...
    if (sel == 0) {         /* select value 0 */
        ...
        if (GetLD_EXC(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
            ...
            /* Priority for exceptions is: Protection, Unaligned Access, Invalid Opcode. */
            /*
             * Check first for Protection Exception. Possible opcodes are:
             *  BR, JMP/RET, JSR(R), LDB, LDW, LEA, STB, STW
             */
            ...
            /* 
             * Check for Unaligned Access. Possible opcodes are: 
             *  LDW, STW
             */
            int exc_unaligned = 0;
            if (opcode == 6) {          /* LDW */
            ...
            /* Priority goes to Protection Exception */
            if (exc_protection == 1) { MUX_OUTPUTS.EXCMUX = 2; }
            else if (exc_unaligned == 1) { MUX_OUTPUTS.EXCMUX = 3; }
