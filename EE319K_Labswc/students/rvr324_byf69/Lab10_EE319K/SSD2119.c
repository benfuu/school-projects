...
//
// Modified by Mark Meserve (mark.meserve@utexas.edu) 4/21/14
//
...
//used to store the color table read in from a BMP file
...
#define BMP_COLOR_TABLE_ENTRIES_OFFSET 0x002E
...
void LCD_DrawBMP(const unsigned char* imgPtr, unsigned short x, unsigned short y){
    short i, j, bpp, headerSize, colorTableCount, rowPadding;
    long width, height, dataOffset;
    const unsigned char* pixelOffset;
		const unsigned char* colorTableOffset;
    
    // read BMP metadata

		//width and height are actually longs per the bmp format
		//shorts are used because they will never be more than 16-bits here
		//also if you use long the compiler optimization creates an unaligned LDRD that hard faults, go figure
	
		//height can also be negative, indicating rows are stored top-to-bottom, we ignore this
    width = *((const unsigned short*)(imgPtr + BMP_WIDTH_OFFSET));
    height = *((const unsigned short*)(imgPtr + BMP_HEIGHT_OFFSET));
    bpp = *((const unsigned short*)(imgPtr + BMP_BPP_OFFSET));
    dataOffset = *((const unsigned long*)( imgPtr + BMP_DATA_OFFSET));
	
		// (actual width of row) - (how many bytes are required for row)
		rowPadding = (((bpp*width+31)>>5)<<2) - ((bpp*width+7)>>3);
		
		//colorTable comes immediately after the header
		headerSize = *(imgPtr + BMP_HEADER_SIZE_OFFSET);
		colorTableOffset = imgPtr + headerSize + BMP_HEADER_SIZE_OFFSET;
	
		if (bpp <= 8)
		{
			colorTableCount = *(imgPtr + BMP_COLOR_TABLE_ENTRIES_OFFSET);
			//if color table entries is 0, indicates 2^bpp
			if (colorTableCount == 0)
				colorTableCount = 1<<bpp;
			for (i = 0; i < colorTableCount; ++i)
			{
				colorTable[i] = convertColor(
												*(colorTableOffset + (4*i) + 2),
												*(colorTableOffset + (4*i) + 1),
												*(colorTableOffset + (4*i) + 0));				
			}
		}
        ...
        switch(bpp){
            ...
            {   // unknown if working yet
                for (j = 0; j < width/8; j++) {
                    ...
                    LCD_WriteData(colorTable[(pixelData&0x80)>>7]);            
                    LCD_WriteData(colorTable[(pixelData&0x40)>>6]);            
                    LCD_WriteData(colorTable[(pixelData&0x20)>>5]);            
                    LCD_WriteData(colorTable[(pixelData&0x10)>>4]);            
                    LCD_WriteData(colorTable[(pixelData&0x08)>>3]);            
                    LCD_WriteData(colorTable[(pixelData&0x04)>>2]);            
                    LCD_WriteData(colorTable[(pixelData&0x02)>>1]);            
                    LCD_WriteData(colorTable[(pixelData&0x01)>>0]);            
                    pixelOffset++;
                } 
								for (j = 0; j < width%8; ++j)
								{
										LCD_WriteData(colorTable[((*(pixelOffset))&(0x80>>j)) >> (7-j)]);
								}
								if (width%8)
								{
										pixelOffset++;
            ...
            {   // working?
                for (j = 0; j < width/2; j++) {
                    unsigned char pixelData = *(pixelOffset);
                    LCD_WriteData( colorTable[(pixelData&0xF0)>>4] );            
										LCD_WriteData( colorTable[pixelData&0x0F] );
                    pixelOffset++;
                } 
								if (width%2) {
										LCD_WriteData( colorTable[(*(pixelOffset)&0xF0)>>4] );
										pixelOffset++;
								}
								break;
            }
						...
						{   // surely this works
								for (j = 0; j < width; j++) {
										unsigned char pixelData = *(pixelOffset);
										LCD_WriteData(colorTable[pixelData&0xFF]);
										pixelOffset++;
								}
						} break;
            case 24:
            {   // seems to work
				...
				//move to the next 4 byte boundary for the start of the row
