...
void ADC_Init(void){ volatile unsigned long delay;
  ...
  ADC0_ACTSS_R |= 0x0008;         // 13) enable sample sequencer 3
	ADC0_SAC_R = ADC_SAC_AVG_64X;		// 14) configure ADC0 to sample 64 times before returning a result
}
...
unsigned long ADC_In(void){	unsigned long answer;
	ADC0_PSSI_R = 0x0008;						//start the ADC
	while((ADC0_RIS_R&0x08)==0){};	//wait until conversion is done to read the data
	answer = ADC0_SSFIFO3_R&0xFFF;	//record the answer into "answer"
	ADC0_ISC_R = 0x0008;						//clear the complete flag by writing to ISC bit 3
  return answer;
}
