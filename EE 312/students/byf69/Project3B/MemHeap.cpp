 ...
 *
 * OK, this implementation uses a "next_fit" allocation
 * THAT'S DIFFERENT FROM WHAT IS USED ON THE EXAMS (which use first fit)!!!!
 */
unsigned findFreeChunk(int size) {
	static unsigned recent_chunk = 0;
	unsigned next_chunk = recent_chunk;
	...
	if (next_chunk == HEAP_SIZE && recent_chunk != 0) {
		next_chunk = 0;
		while (next_chunk < recent_chunk && memory_pool[next_chunk] < size) {
			/* this chunk is too small (or in use), try next chunk */
			next_chunk = successorChunk(next_chunk);
		}
	}
	 ...
	 */
	if (next_chunk == recent_chunk && memory_pool[next_chunk] < size) { // uh oh! gone too far, give up
		return bogus_chunk;
	} else {
		recent_chunk = next_chunk; 
		return next_chunk;
	}
}
