...
void LifeForm::update_position(void) {
	if (!this->is_alive) { return; }
	if (!space.is_occupied(this->pos)) {
		cout << "trying to update a position that does not exist in the tree" << endl;
		//return;
	}
...
void LifeForm::check_encounter(void) {
    ...
    /*
	in_radius = space.nearby(this->pos, encounter_distance);	// update vector with current positions
	SmartPointer<LifeForm> closest;
	if (in_radius.size() > 0) {	// choose the closest one
		closest = space.closest(this->pos);
		if (closest && closest->pos.distance(this->pos) <= encounter_distance) {
			this->resolve_encounter(closest);
		}
	}
...
void LifeForm::reproduce(SmartPointer<LifeForm> child) {
	...
	if (elapsed_time > min_reproduce_time) {	// reproduce must not be called more frequently than min_reproduce_time
		std::uniform_real_distribution<double> distribution(encounter_distance, reproduce_dist);	// generate number between encounter_distance and reproduce_dist
